...
async function main() {
  ...
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), 'quark-runtime-update-')));
...
app.whenReady().then(async () => {
  ...
  await window.loadURL('data:text/html,<title>Update fixture</title>');
...
`);
  ...
  // Reproduce an interrupted staging copy before retrying the real helper.
  const incoming = path.join(root, '.quark-update-next-' + id);
  await fs.mkdir(incoming);
  await fs.writeFile(path.join(incoming, spec.executable), 'interrupted copy');
  await fs.writeFile(path.join(work, 'plan.json'), JSON.stringify({ id, version: '1.2.0', oldVersion: '1.1.0',
    platform: 'win32', arch: 'x64', parentPid: process.pid, installRoot: await fs.realpath(installRoot),
    incoming: await fs.realpath(incoming), backup: path.join(await fs.realpath(root), '.quark-update-previous-' + id), work: await fs.realpath(work) }));
  // Explorer and portable shortcuts commonly start the app in its own folder.
  // A detached update helper must not inherit that Windows directory handle.
  const child = spawn(path.join(installRoot, spec.executable), [], { cwd: installRoot, windowsHide: true, stdio: 'ignore' });
  ...
  if (!inside(await fs.realpath(os.tmpdir()), root) || !path.basename(root).startsWith('quark-runtime-update-')) throw new Error('Unsafe fixture cleanup path');
  ...
  console.log('PASS: real Windows helper recovered interrupted staging, replaced an app started from its own directory with renderer/GPU processes, received the new version boot acknowledgement and removed the recovery copy.');
}
